package org.lys.demo.encode.asymmetric_encryption;
...
public class ElGamalTest {
	...
	public void testJDK() {
		try {
			...
			/*
			 * // 私钥加密、公钥解密: 有问题 // 4.私钥加密、公钥解密 ---- 加密 PKCS8EncodedKeySpec
			 * pkcs8EncodedKeySpec = new
			 * PKCS8EncodedKeySpec(elGamalPrivateKey.getEncoded()); KeyFactory
			 * keyFactory = KeyFactory.getInstance("Elgamal"); PrivateKey
			 * privateKey = keyFactory.generatePrivate(pkcs8EncodedKeySpec);
			 * Cipher cipher = Cipher.getInstance("Elgamal");
			 * cipher.init(Cipher.ENCRYPT_MODE, privateKey); byte[] result =
			 * cipher.doFinal(src.getBytes()); System.out.println(
			 * "私钥加密、公钥解密 ---- 加密:" + Base64.encodeBase64String(result));
			 * 
			 * // 5.私钥加密、公钥解密 ---- 解密 X509EncodedKeySpec x509EncodedKeySpec =
			 * new X509EncodedKeySpec(elGamalPublicKey.getEncoded()); keyFactory
			 * = KeyFactory.getInstance("Elgamal"); PublicKey publicKey =
			 * keyFactory.generatePublic(x509EncodedKeySpec); cipher =
			 * Cipher.getInstance("Elgamal"); cipher.init(Cipher.DECRYPT_MODE,
			 * publicKey); result = cipher.doFinal(result); System.out.println(
			 * "Elgamal 私钥加密、公钥解密 ---- 解密:" + new String(result));
			 */
		} catch (Exception e) {
			e.printStackTrace();
		}
